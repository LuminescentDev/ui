import type React from 'react';
import { getClasses } from '../functions';

export interface ToggleProps extends Omit<
  React.InputHTMLAttributes<HTMLInputElement>,
  'type'
> {
  checkbox?: boolean;
  round?: boolean;
  outerProps?: React.HTMLAttributes<HTMLDivElement>;
  children?: React.ReactNode;
}

export function Toggle({
  className,
  checkbox,
  round,
  outerProps,
  children,
  ...props
}: ToggleProps) {
  return (
    <div
      {...outerProps}
      className={getClasses({
        'flex touch-manipulation items-center gap-3': true,
        [outerProps?.className ?? '']: !!outerProps?.className,
      })}
    >
      <label className="relative inline-flex cursor-pointer items-center">
        <input type="checkbox" {...props} className="peer sr-only" />
        <div
          className={getClasses({
            'peer h-7 duration-300 ease-out hover:duration-75 motion-safe:transition-all': true,
            "after:absolute after:top-1 after:left-1 after:h-5 after:w-5 after:border after:content-[''] after:transition-all after:duration-300 after:ease-out": true,
            'rounded-lum after:rounded-lum-1': !round,
            'rounded-full after:rounded-full': round,
            'w-12 after:translate-x-0 peer-checked:after:translate-x-full':
              !checkbox,
            'w-7 after:opacity-0 peer-checked:after:opacity-100': checkbox,
            'lum-toggle-bg-lum-input-bg peer-checked:lum-toggle-bg-lum-accent': true,
            [className ?? '']: !!className,
          })}
        />
      </label>
      {children && (
        <label
          htmlFor={props.id}
          className="text-lum-text flex gap-2 select-none empty:hidden"
        >
          {children}
        </label>
      )}
    </div>
  );
}
